...
def show_test_predictions(testfolder='../data/test/',predictionsfloder='../data/predictions/'):
#def show_test_predictions(testfolder='../data/train/', predictionsfloder='../data/train/'):
    filename = 'Case00'
    itkimage = sitk.ReadImage(testfolder + filename + '.mhd')
    dest = '../images/'+ filename
    imgs = sitk.GetArrayFromImage(itkimage)
    itkimage_pred = sitk.ReadImage(predictionsfloder + filename + '_segmentation.mhd')
    imgs_pred = sitk.GetArrayFromImage(itkimage_pred)
    for i in range(0, len(imgs)):
        img = img_resize(imgs, 256, 256, equalize=False)[i]
        img_pred = img_resize(imgs_pred, 256, 256, equalize=False)[i]
        plt.figure()
